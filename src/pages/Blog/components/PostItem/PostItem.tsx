...
const PostItem = (props: IPostWithAction) => {
  const { description, images, _id, publish, title, handleRemovePost, handleEditPost } = props
  return (
    <div className=' space-y-12 '>
      ...
      <div className='flex gap-2'>
        <button className='px-4' onClick={() => handleRemovePost(_id)}>
          Remove
        </button>
        <button onClick={() => handleEditPost(_id)} className='border border-green-500 px-3 text-green-500'>
          Sửa
        </button>
      </div>
    </div>
